...
def encoderVAE(inputs, hiden_size):
    with tf.variable_scope("foo"):
        l1 = add_layer(inputs, 3, 3, activation_function=tf.nn.sigmoid)
...
def decoderVAE(inputs, hiden_size):
    ...
    with tf.variable_scope("foo8"):
        prediction = add_layer(l3,3, 3)#,activation_function=tf.nn.sigmoid)
    return prediction
...
train_step1 = tf.train.GradientDescentOptimizer(0.00000001).minimize(lossFinal1+lossFinal2)
...
disReal=60*flagReal+flagRealPure*10
